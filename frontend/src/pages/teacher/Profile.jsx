...
export default function Profile() {
  ...
  const { profile, loading, error, refetch, setProfile } = useTeacherProfile()
  ...
  async function handleProfileSubmit(event) {
    ...
    try {
      const response = await teacherApi.updateProfile(personalForm)
  ...
  async function handlePasswordSubmit(event) {
    ...
    try {
      const response = await teacherApi.changePassword(passwordForm)
    ...
    <div className="space-y-6">
      <header className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-2 text-sm text-slate-600">
          View and update your teacher profile and account security.
        </p>
      </header>
      ...
      <ProfileSection title="Account Information" description="Your teacher account details.">
        <dl className="grid gap-6 sm:grid-cols-2">
          <ReadOnlyField label="Full Name" value={profile.full_name} />
          <ReadOnlyField label="Teacher ID" value={profile.employee_id} />
