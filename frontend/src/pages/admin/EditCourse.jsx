...
import { useAdminCourseDetail } from '../../hooks/useAdminData'
import adminApi from '../../services/admin'

export default function EditCourse() {
  const { courseId } = useParams()
  const navigate = useNavigate()
  const { course, loading, error } = useAdminCourseDetail(courseId)
  const [values, setValues] = useState({ title: '', code: '', description: '' })
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await adminApi.updateCourse(courseId, values)
      setSuccessMessage(response.message || 'Course updated successfully')
      setTimeout(() => {
        navigate(`${ADMIN_BASE_PATH}/courses`)
      }, 800)
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-2 text-sm text-slate-600">Update the course information.</p>
        ...
        <div className="mt-6 space-y-4">
          ...
          <CourseForm
            ...
            onCancel={() => navigate(`${ADMIN_BASE_PATH}/courses`)}
          />
        </div>
      </section>
    </div>
