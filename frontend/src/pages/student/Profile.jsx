...
export default function Profile() {
  ...
  const { profile, loading, error, refetch, setProfile } = useStudentProfile()
  ...
  async function handleProfileSubmit(event) {
    ...
    try {
      const response = await studentApi.updateProfile(personalForm)
  ...
  async function handlePasswordSubmit(event) {
    ...
    try {
      const response = await studentApi.changePassword(passwordForm)
    ...
    <div className="space-y-6">
      <header className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-2 text-sm text-slate-600">
          View your account details and update your profile or password when needed.
        </p>
      </header>
      ...
      >
        <dl className="grid gap-6 sm:grid-cols-2">
          ...
          <ReadOnlyField label="Student ID" value={profile.student_number} />
